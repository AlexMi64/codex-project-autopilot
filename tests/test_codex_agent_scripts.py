...
class CodexAgentScriptTests(unittest.TestCase):
    ...
    def test_init_creates_ultra_context_and_phase_aware_packs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(state["security_profile"])
            self.assertTrue(state["security_guardrails"])
            ...
            self.assertIn("Безопасные дефолты", context_bundle)
    ...
    def test_init_detects_secondary_archetypes_and_capabilities(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("least privilege", state["security_profile"]["access_policy"])
